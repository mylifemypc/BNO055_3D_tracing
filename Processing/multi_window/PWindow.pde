...
class PWindow extends PApplet {
  ...
  void setup() {
    ...
    model = new OBJModel(this, "C:/Users/Nobat/Documents/Processing/libraries/OBJLoader/examples/OBJLoader_Multi_Models_MATTD/data/cassini.obj", "relative", TRIANGLES);
  ...
  ac_Label = new GLabel(this, 550, 20, 350, 25, "Calibration: Sys=? Gyro=? Accel=? Mag=?");
  configPanel.addControl(ac_Label);
  or_Label = new GLabel(this, 800, 20, 350, 25, "Calibration: Sys=? Gyro=? Accel=? Mag=?");
  configPanel.addControl(or_Label);
  ...
  pointLight(255, 255, 255,    0, 800, -500);
...
void serialEvent(Serial p) 
{
  ...
  if ((incoming.length() > 8))
  {
    ...
    if ( (list.length > 0) && (list[0].equals("Orientation:")) ) 
    {
      ...
      or_Label.setText("Orientation: X=" + yaw + " Y=" + pitch + " Z=" + roll);
    }
    if ( (list.length > 0) && (list[0].equals("LINEARACCEL:")) ) 
    {
      ac_roll  = float(list[3]); // Roll = Z
      ac_pitch = float(list[2]); // Pitch = Y 
      ac_yaw   = float(list[1]); // Yaw/Heading = X
      ac_Label.setText("LINEARACCEL: X=" + ac_yaw + " Y=" + ac_pitch + " Z=" + ac_roll);
    }
